...
export default async function GamesLobby() {
  ...
  const [recentWinners, profileStats] = await Promise.all([
    canPlay && defaultContractId
      ? fetchRecentWinners(5, defaultContractId)
      : Promise.resolve<RecentWinnerEntry[]>([]),
    canPlay ? fetchProfileStatsForSession(session) : Promise.resolve(null),
  ]);
            ...
            return (
              <Card key={game.id} hover elevated>
                <CardContent className="p-8 space-y-6">
                  ...
                  {canPlay ? (
                    <a
                      href={`/app/games/slots?contract=${game.contract_id}`}
                      className="block"
                    >
